...
buildings-own [ bpopularity ]

skyscrapers-own [ spopularity ]
...
to setup
  ...
  set-default-shape skyscrapers "house"
...
to toggle-building
  ...
  let nearby-skyscrapers skyscrapers in-radius 4
  ifelse any? nearby-buildings or any? nearby-skyscrapers [
    ; if there is a building near where the mouse was clicked
    ; (and there should always only be one), we remove it and
    ifelse any? nearby-skyscrapers [
      ask nearby-skyscrapers [ die ]
    ] [
    ...
    ifelse random 10 < 3 [
      sprout-skyscrapers 1 [
        set color blue
        set size 5
        set spopularity 1000
      ]
    ] [
      sprout-buildings 1 [
        set color red
  ...
  if count skyscrapers > 0 [
    ask skyscrapers [ obtain-links ]
  ]
  if count buildings > 0 [
    ask buildings [ obtain-links ]
  ]
end

to obtain-links
  let nearby-buildings buildings in-radius 30
  let nearby-skyscrapers skyscrapers in-radius 30
  create-links-with other nearby-buildings
  create-links-with other nearby-skyscrapers
end
...
to decay-popularity
  ...
  ask buildings with [ bpopularity > 0 ] [
    set bpopularity bpopularity - 1
    if bpopularity < 1 [ die ]
  ]
  ask skyscrapers with [ spopularity > 0 ] [
    set spopularity spopularity - 1
    if spopularity < 1 [ die ]
  ]
end
...
to move-walkers
  ask walkers [
    ifelse patch-here = goal [
      ifelse count buildings + count skyscrapers >= 2 [
        ifelse count buildings > 0 [
          ifelse count skyscrapers > 0 [
            ifelse random 10 < 4 [
              set goal [ patch-here ] of one-of buildings
            ] [
              set goal [ patch-here ] of one-of skyscrapers
            ]
          ] [
            set goal [ patch-here ] of one-of buildings
          ]
...
210
525
Buildings ans Skyscrapers popularity
ticks
count
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count buildings"
"pen-1" 1.0 0 -7500403 true "" "plot count skyscrapers"
